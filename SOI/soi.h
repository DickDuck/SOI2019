...
// from here: http://louisdx.github.io/cxx-prettyprint/
...
// test runner: if CONSOLE is defined, read from console
// otherwise, read from sample01 to sample99
...
#ifndef MAX_SAMPLE
#define MAX_SAMPLE 99 // check sample01 to sample99, inclusively
#endif

    bool ok = false;
    for (int i = 1; i <= MAX_SAMPLE; ++i) {
        std::ostringstream ss;
        ss << "sample" << std::setw(2) << std::setfill('0') << i << ".in";
        const std::string sample = ss.str();
        std::ifstream fin(sample);
        if (!fin)
            continue;
        ok = true;
        std::cout << "--- "
                  << std::left << std::setw(50 - 5) << std::setfill('-') << sample + ' '
                  << std::right << '\n';
        std::streambuf *cinbuf = std::cin.rdbuf(fin.rdbuf()); // redirect std::cin to fin
        ...
        std::cin.rdbuf(cinbuf); // restore original cin buffer
    }
    if (!ok)
        std::cerr << "WARNING: no sample file found.  Create sample01.in in this directory.\n";
#endif
}

// =============================================================================
// make certain functions unusable
